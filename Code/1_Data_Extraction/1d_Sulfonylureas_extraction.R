...
#
# Inputs:
# 1) Stephanie/SMI_GLP/Code_Lists/Sulfonylureas/Aurum_Sulfonylureas_codelist_20250614.txt: Updated Aurum Sulfonylureas code list
# 2) Stephanie/SMI_GLP/Code_Lists/Sulfonylureas/Gold_Sulfonylureas_codelist_20250614.txt: Updated GOLD Sulfonylureas code list
...
# GOLD code list
su_gold <- read_delim(
  file = paste0(wd, path_input, "Gold_Sulfonylureas_codelist_20250614.txt"), 
  delim = "\t", escape_double = FALSE, 
  col_types = cols(prodcode = col_character()),  trim_ws = TRUE) 

# AURUM code list
su_aurum <- read_delim(
  file = paste0(wd, path_input, "Aurum_Sulfonylureas_codelist_20250614.txt"), 
  delim = "\t", escape_double = FALSE, 
...
n_distinct(pat_su_gold$patid) # 8,542
...
# Number of unique patients with condition
n_distinct(pat_su_aurum$patid) # 19,882
...
pat_su_comb <- pat_su_comb %>%
  ...
  mutate(
    patid = case_when(
      ...
      .default = patid)) %>%
  distinct()  # Removed 2161 duplicates. 1,981,351 remaining


# Number of unique patients with condition
n_distinct(pat_su_comb$patid) # 28,422
