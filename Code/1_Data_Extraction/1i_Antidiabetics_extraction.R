...
gold_file_name <- list.files(path = paste0(wd, path_input),
                             pattern = "^Gold_Antidiabetics_codelist")
# Check date
gold_file_name
...
aurum_file_name <- list.files(path = paste0(wd, path_input),
                              pattern = "^Aurum_Antidiabetics_codelist")
# Check date
aurum_file_name
...
pat_antidiabetics_gold <- pat_antidiabetics_gold_therapy %>%
...
# load(file = paste0(wd, path_output, "Extraction_Files/pat_antidiabetics_aurum.RData"))
# load(file = paste0(wd, path_output, "Extraction_Files/pat_antidiabetics_gold.RData"))
...
pat_antidiabetics_comb <- pat_antidiabetics_aurum_lookup %>% 
  # Add in GOLD patient files
  bind_rows(pat_antidiabetics_gold_lookup)

# Transform dates and exclude entries with invalid Antidiabetics dates
...
pat_antidiabetics_comb <- pat_antidiabetics_comb %>%
  ...
  mutate(
    patid = case_when(
      ...
      .default = patid)) %>%
  distinct()  # Removed 6,553 duplicates. 9,723,357 remaining


# Number of unique patients with condition
n_distinct(pat_antidiabetics_comb$patid) # 68,677

# # Save patient data for GOLD and Aurum
# save(pat_antidiabetics_comb,
#      file = paste0(wd, path_output, "Extraction_Files/pat_antidiabetics_comb.RData"))
