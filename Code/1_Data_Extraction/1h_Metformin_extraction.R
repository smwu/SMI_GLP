# ==============================================================================
# Extract patients with Metformin medications using code lists
# Author: SM Wu
...
#
# Inputs:
# 1) Stephanie/SMI_GLP/Code_Lists/Metformin/Aurum_Metformin_codelist_20250704.txt: Updated Aurum Metformin code list
# 2) Stephanie/SMI_GLP/Code_Lists/Metformin/Gold_Metformin_codelist_20250704.txt: Updated GOLD Metformin code list
...
# 1) Stephanie/SMI_GLP/Data/Extraction_Files/pat_metformin_gold.RData: GOLD patient files for Metformin medication
# 2) Stephanie/SMI_GLP/Data/Extraction_Files/pat_metformin_aurum.RData: Aurum patient files for Metformin medication
# 
# Final Outputs:
# 1) Stephanie/SMI_GLP/Data/Extraction_Files/pat_metformin_comb.RData: Combined GOLD and Aurum patient files for Metformin medication
...
path_input <- "Stephanie/SMI_GLP/Code_Lists/Metformin/"
...
# GOLD code list
metformin_gold <- read_delim(
  file = paste0(wd, path_input, "Gold_Metformin_codelist_20250704.txt"), 
  delim = "\t", escape_double = FALSE, 
  col_types = cols(prodcode = col_character()),  trim_ws = TRUE) 

# AURUM code list
metformin_aurum <- read_delim(
  file = paste0(wd, path_input, "Aurum_Metformin_codelist_20250704.txt"), 
  delim = "\t", escape_double = FALSE, 
...
pat_metformin_gold_therapy <- read_obs_condition(
  file_path = paste0(wd, "2023 CPRD/GOLD/Therapy/"),
  file_names = gold_therapy_files,
  code_list = metformin_gold,
  database = "gold",
  medcode = FALSE)

# Create new column to indicate database
pat_metformin_gold <- pat_metformin_gold_therapy %>%
  mutate(database = "Gold")

# Number of unique patients with condition
n_distinct(pat_metformin_gold$patid) # 16,442

# # Save extracted patient files matching code list conditions 
# save(pat_metformin_gold,
#      file = paste0(wd, path_output, "Extraction_Files/pat_metformin_gold.RData"))
...
# Extract patient files matching conditions from code list
pat_metformin_aurum_drug <- read_obs_condition(
  file_path = paste0(wd, "2023 CPRD/Aurum/DrugIssue/"),
  file_names = aurum_drug_files,
  code_list = metformin_aurum,
  database = "aurum",
  medcode = FALSE)

# Create new column to indicate database
pat_metformin_aurum <- pat_metformin_aurum_drug %>%
  mutate(database = "Aurum")

# Number of unique patients with condition
n_distinct(pat_metformin_aurum$patid) # 13,887

# # Save extracted patient files matching code list conditions 
# save(pat_metformin_aurum,
#      file = paste0(wd, path_output, "Extraction_Files/pat_metformin_aurum.RData"))



# ================= 4) Add in look up information ==============================
# # Load extraction files if necessary
# load(file = paste0(wd, path_output, "Extraction_Files/pat_metformin_aurum.RData"))
# load(file = paste0(wd, path_output, "Extraction_Files/pat_metformin_gold.RData"))
...
pat_metformin_gold_lookup <- pat_metformin_gold %>%
...
pat_metformin_aurum_lookup <- pat_metformin_aurum %>%
  # Add in common dosages
  left_join(common_dosages_g, by = "dosageid") %>%
  select(-dosageid) %>%
...
pat_metformin_comb <- pat_metformin_aurum_lookup %>% 
  # Add in GOLD patient files
  bind_rows(pat_metformin_gold_lookup)

# Transform dates and exclude entries with invalid Metformin dates
# 0 excluded. 1,603,098 remaining
pat_metformin_comb <- transform_dates_meds(patient_data = pat_metformin_comb,
                                         earliest_date = '1900-01-01',
                                         latest_date = '2023-06-01')
# Rearrange columns, add Gold and Aurum identifiers to patid, and drop duplicates
pat_metformin_comb <- pat_metformin_comb %>%
...
# Number of unique patients with condition
n_distinct(pat_metformin_comb$patid) # 19,463

# # Save patient data for GOLD and Aurum
# save(pat_metformin_comb,
#      file = paste0(wd, path_output, "Extraction_Files/pat_metformin_comb.RData"))
