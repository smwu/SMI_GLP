...
# 1) Stephanie/SMI_GLP/Data/Extraction_Files/pat_dpp4i_gold.RData: GOLD patient files for DPP4is medication
# 2) Stephanie/SMI_GLP/Data/Extraction_Files/pat_dpp4i_aurum.RData: Aurum patient files for DPP4is medication
# 
# Final Outputs:
# 1) Stephanie/SMI_GLP/Data/Extraction_Files/pat_dpp4i_comb.RData: Combined GOLD and Aurum patient files for DPP4is medication
...
path_input <- "Stephanie/SMI_GLP/Code_Lists/DPP4is/"
...
# save(pat_dpp4i_gold,
#      file = paste0(wd, path_output, "Extraction_Files/pat_dpp4i_gold.RData"))
...
# Extract patient files matching conditions from code list
pat_dpp4i_aurum_drug <- read_obs_condition(
  file_path = paste0(wd, "2023 CPRD/Aurum/DrugIssue/"),
  file_names = aurum_drug_files,
  code_list = dpp4i_aurum,
  database = "aurum",
  medcode = FALSE)

# Create new column to indicate database
pat_dpp4i_aurum <- pat_dpp4i_aurum_drug %>%
...
#      file = paste0(wd, path_output, "Extraction_Files/pat_dpp4i_aurum.RData"))



# ================= 4) Add in look up information ==============================
# # Load extraction files if necessary
# load(file = paste0(wd, path_output, "Extraction_Files/pat_dpp4i_aurum.RData"))
# load(file = paste0(wd, path_output, "Extraction_Files/pat_dpp4i_gold.RData"))
...
pat_dpp4i_gold_lookup <- pat_dpp4i_gold %>%
...
pat_dpp4i_aurum_lookup <- pat_dpp4i_aurum %>%
  # Add in common dosages
  left_join(common_dosages_g, by = "dosageid") %>%
  select(-dosageid) %>%
...
pat_dpp4i_comb <- pat_dpp4i_aurum_lookup %>% 
...
# save(pat_dpp4i_comb,
#      file = paste0(wd, path_output, "Extraction_Files/pat_dpp4i_comb.RData"))
