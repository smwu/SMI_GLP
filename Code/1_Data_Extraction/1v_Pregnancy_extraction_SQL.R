# ==============================================================================
# Extract patients with Pregnancy diagnoses using code lists (SQL Version)
# Author: SM Wu
...
#
# Inputs:
# 1) ~/SMI_GLP/Code_Lists/Pregnancy/Aurum_Pregnancy_codelist_20250929.txt: Updated Aurum Pregnancy code list
# 2) ~/SMI_GLP/Code_Lists/Pregnancy/Gold_Pregnancy_codelist_20250929.txt: Updated GOLD Pregnancy code list
...
# 1) ~/SMI_GLP/Data/Extraction_Files/pat_pregnancy_gold.RData: GOLD patient files for Pregnancy diagnosis
# 2) ~/SMI_GLP/Data/Extraction_Files/pat_pregnancy_aurum.RData: Aurum patient files for Pregnancy diagnosis
# 
# Final Outputs:
# 1) ~/SMI_GLP/Data/Extraction_Files/pat_pregnancy_comb.RData: Combined GOLD and Aurum patient files for Pregnancy diagnosis
...
# Read in pregnancy codelists
# GOLD code list
gold_file_name <- list.files(path = paste0(wd, path_input), pattern = "^Gold_Pregnancy_codelist")
# Check date
gold_file_name
...
# AURUM code list
aurum_file_name <- list.files(path = paste0(wd, path_input), pattern = "^Aurum_Pregnancy_codelist")
# Check date
aurum_file_name
...
DBI::dbExecute(connection, "
  ...
  SELECT 
    medcode, term, patid, eventdate, sysdate, constype, consid, database, source, period
  FROM gold_clin_raw
  UNION ALL SELECT 
    medcode, term, patid, eventdate, sysdate, constype, consid, database, source, period
  FROM gold_test_raw
...
DBI::dbExecute(connection, "
  ...
  SELECT DISTINCT
    patid || '-G' AS patid,
    medcode, term, eventdate, sysdate, constype, consid, database, source, period
  FROM pat_gold_clean;
")
