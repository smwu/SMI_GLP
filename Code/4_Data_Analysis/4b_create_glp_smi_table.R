...
# out_docx <- paste0(wd, path_output, "Tables/socio_glp_smi_", today(), ".docx")
# out_csv  <- paste0(wd, path_output, "Tables/socio_glp_smi_", today(), ".csv")

# Load in helper functions
source(paste0(wd, "SMI_GLP/Code/4_Data_Analysis/",
              "4_helper_functions.R"))

# Read in T2DM cohort. Loads as 'cohort_demog_dx_date_smi'
load(paste0(path_input, "cohort_demog_cleaned_with_smi.RData"))
head(cohort_demog_dx_date_smi)
cohort_patids <- cohort_demog_dx_date_smi %>% select(patid)
