# ----------------
# Lipid regulating drugs code list
...
# To be called as: aurum_matches_df <- match_medications_2(aurum_matches_df, "lipidregulator", medication_reference$lipid_regulating_drugs)
...
#Medication names
medication_reference <- read_excel("Data files/Misc/medication_reference.xlsx", 
                                   sheet = "lipid_regulating_drugs", skip = 1) %>%
  rename(lipid_regulating_drugs = Clean) %>%
  mutate(lipid_regulating_drugs = str_to_lower(lipid_regulating_drugs)) %>%
  filter(!is.na(lipid_regulating_drugs) & is.na(Exclude)) %>%
  select(lipid_regulating_drugs, 'Brand names')
...
brands <- medication_reference %>% 
  ...
  select(lipid_regulating_drugs, brandnames) %>%
...
CPRDAurumProduct$lipid_regulating_drugs <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "lipid_regulating_drugs" columns in CPRDAurumProduct
  CPRDAurumProduct$brandnames[matches] <- brandnames
  CPRDAurumProduct$lipid_regulating_drugs[matches] <- brands$lipid_regulating_drugs[brands$brandnames == brandnames]}

CPRDAurumProduct <- CPRDAurumProduct %>%
  mutate(ingredient = coalesce(ingredient, lipid_regulating_drugs)) %>%
...
CPRDGoldProduct$lipid_regulating_drugs <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "lipid_regulating_drugs" columns in CPRDGoldProduct
  CPRDGoldProduct$brandnames[matches] <- brandnames
  CPRDGoldProduct$lipid_regulating_drugs[matches] <- brands$lipid_regulating_drugs[brands$brandnames == brandnames]}
...
#Search for matches
aurum_matches <- match_medications_1(CPRDAurumProduct, medication_reference$lipid_regulating_drugs)
#Assign matches to df
aurum_matches_df <- CPRDAurumProduct[aurum_matches, ]
#Add medication name column
aurum_matches_df <- match_medications_2(aurum_matches_df, "LipidRegulatingMed", medication_reference$lipid_regulating_drugs)
...
# save as text file
write.table(aurum_matches_df, file = "Code lists/Lipid drugs/CPRD-2023/lipidregulatingdrugs_AURUM_210723.txt",
            sep = "\t", row.names = FALSE)
...
#Search for matches
gold_matches <- match_medications_1(CPRDGoldProduct, medication_reference$lipid_regulating_drugs)
#Assign matches to df
gold_matches_df <- CPRDGoldProduct[gold_matches, ]
#Add medication name column
gold_matches_df <- match_medications_2(gold_matches_df, "LipidRegulatingMed", medication_reference$lipid_regulating_drugs)
#Filter to more precise matches
...
# save as text file
write.table(gold_matches_df, file = "Code lists/Lipid drugs/CPRD-2023/lipidregulatingdrugs_GOLD_210723.txt",
            sep = "\t", row.names = FALSE)
