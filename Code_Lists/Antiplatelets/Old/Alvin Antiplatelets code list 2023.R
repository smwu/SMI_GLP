...
# To be called as: aurum_matches_df <- match_medications_2(aurum_matches_df, "lipidregulator", medication_reference$antiplatelet)
...
medication_reference <- read_excel(paste0(path, "/ritd-ag-project-rd00qv-jfhay18/Alvin/Data files/Misc/medication_reference.xlsx"), 
  ...
  rename(antiplatelet = Clean) %>%
  mutate(antiplatelet = str_to_lower(antiplatelet)) %>%
  filter(!is.na(antiplatelet) & is.na(Exclude)) %>%
  select(antiplatelet, 'Brand names')
...
brands <- medication_reference %>% 
  ...
  select(antiplatelet, brandnames) %>%
...
CPRDAurumProduct$antiplatelet <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "antiplatelet" columns in CPRDAurumProduct
  CPRDAurumProduct$brandnames[matches] <- brandnames
  CPRDAurumProduct$antiplatelet[matches] <- brands$antiplatelet[brands$brandnames == brandnames]}

CPRDAurumProduct <- CPRDAurumProduct %>%
  mutate(ingredient = coalesce(ingredient, antiplatelet)) %>%
...
CPRDGoldProduct$antiplatelet <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "antiplatelet" columns in CPRDGoldProduct
  CPRDGoldProduct$brandnames[matches] <- brandnames
  CPRDGoldProduct$antiplatelet[matches] <- brands$antiplatelet[brands$brandnames == brandnames]}
...
#Search for matches
aurum_matches <- match_medications_1(CPRDAurumProduct, medication_reference$antiplatelet)
#Assign matches to df
aurum_matches_df <- CPRDAurumProduct[aurum_matches, ]
#Add medication name column
aurum_matches_df <- match_medications_2(aurum_matches_df, "antiplatelet", medication_reference$antiplatelet)
...
# save as text file
write.table(aurum_matches_df, file = "Code lists/Antiplatelets/antiplatelet_AURUM_250124.txt",
            sep = "\t", row.names = FALSE)
...
#Search for matches
gold_matches <- match_medications_1(CPRDGoldProduct, medication_reference$antiplatelet)
#Assign matches to df
gold_matches_df <- CPRDGoldProduct[gold_matches, ]
#Add medication name column
gold_matches_df <- match_medications_2(gold_matches_df, "antiplatelet", medication_reference$antiplatelet)
#Filter to more precise matches
...
# save as text file
write.table(gold_matches_df, file = "Code lists/Antiplatelets/antiplatelet_GOLD_250124.txt",
            sep = "\t", row.names = FALSE)
