...
# To be called as: aurum_matches_df <- match_medications_2(aurum_matches_df, "lipidregulator", medication_reference$anticoagulant)
...
medication_reference <- read_excel(paste0(path, "/ritd-ag-project-rd00qv-jfhay18/Alvin/Data files/Misc/medication_reference.xlsx"), 
                                   sheet = "anticoagulants", skip = 1) %>%
  rename(anticoagulant = Clean) %>%
  mutate(anticoagulant = str_to_lower(anticoagulant)) %>%
  filter(!is.na(anticoagulant) & is.na(Exclude)) %>%
  select(anticoagulant, 'Brand names')
...
brands <- medication_reference %>% 
  ...
  select(anticoagulant, brandnames) %>%
...
CPRDAurumProduct$anticoagulant <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "anticoagulant" columns in CPRDAurumProduct
  CPRDAurumProduct$brandnames[matches] <- brandnames
  CPRDAurumProduct$anticoagulant[matches] <- brands$anticoagulant[brands$brandnames == brandnames]}

CPRDAurumProduct <- CPRDAurumProduct %>%
  mutate(ingredient = coalesce(ingredient, anticoagulant)) %>%
...
CPRDGoldProduct$anticoagulant <- NA_character_
...
for (brandnames in brands$brandnames) {
  ...
  # Assign the matching brand name and antipsychotic to the "brandnames" and "anticoagulant" columns in CPRDGoldProduct
  CPRDGoldProduct$brandnames[matches] <- brandnames
  CPRDGoldProduct$anticoagulant[matches] <- brands$anticoagulant[brands$brandnames == brandnames]}
...
#Search for matches
aurum_matches <- match_medications_1(CPRDAurumProduct, medication_reference$anticoagulant)
#Assign matches to df
aurum_matches_df <- CPRDAurumProduct[aurum_matches, ]
#Add medication name column
aurum_matches_df <- match_medications_2(aurum_matches_df, "anticoagulant", medication_reference$anticoagulant)
...
# save as text file
write.table(aurum_matches_df, file = "Code lists/Anticoagulants/anticoagulant_AURUM_310124.txt",
            sep = "\t", row.names = FALSE)
...
#Search for matches
gold_matches <- match_medications_1(CPRDGoldProduct, medication_reference$anticoagulant)
#Assign matches to df
gold_matches_df <- CPRDGoldProduct[gold_matches, ]
#Add medication name column
gold_matches_df <- match_medications_2(gold_matches_df, "anticoagulant", medication_reference$anticoagulant)
#Filter to more precise matches
...
# save as text file
write.table(gold_matches_df, file = "Code lists/Anticoagulants/anticoagulant_GOLD_310124.txt",
            sep = "\t", row.names = FALSE)
